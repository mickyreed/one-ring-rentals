...
const DealCard =({ id, title, location, region, link, image, availability,}) => {
                ...
                <div className="columns  ">
                    ...
                    <div className="column is-half is-flex is-align-items-center pb-4">
                        <div className="is-size-7 mb-0 pr-2 ">
                            <div><i className="fa fa-calendar pr-2">
                            </i>Available {checkDateAvailable(availability)}</div>
                        </div>

                        <h4 className="has-text-black is-size-6 mb-0">
                            <a href={`/deals/${id}`} className={`location-highlight ${isHovered ? 'hovered' : ''}`}
